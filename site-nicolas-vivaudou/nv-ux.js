...
/* ---------------------------------------------------------------------------
   Images manquantes — filet de sécurité global.
   Une image qui n'arrive pas affiche par défaut l'icône « image cassée » et son
   texte alternatif en clair (« sport », « évènement »…), au milieu de la page.
   On intercepte l'échec pour toutes les images du site, en une seule fois :
     1. si une adresse de repli est connue (data-fb, ou l'original contenu dans
        l'URL de redimensionnement), on la tente ;
     2. sinon l'image s'effface proprement, sans texte ni icône.
   La capture est nécessaire : l'évènement « error » d'une image ne remonte pas.
   --------------------------------------------------------------------------- */
document.addEventListener('error', function (ev) {
  var el = ev.target;
  if (!el || el.tagName !== 'IMG' || el.dataset.nvGone) return;
  var url = el.currentSrc || el.getAttribute('src') || '';

  // Adresses de repli : celles fournies par la page, puis l'original contenu
  // dans une URL de redimensionnement Cloudflare.
  var list = (el.getAttribute('data-fb') || '').split('|');
  var m = url.match(/\/cdn-cgi\/image\/[^/]+\/(https?:\/\/.+)$/);
  if (m) list.push(m[1]);
  list = list.filter(function (u) { return u && u !== url; });

  var i = parseInt(el.dataset.nvTry || '0', 10);
  if (i < list.length) {
    el.dataset.nvTry = String(i + 1);
    el.src = list[i];
    return;
  }

  // Plus rien à tenter : on efface, sans icône ni texte alternatif.
  el.dataset.nvGone = '1';
